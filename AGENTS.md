...
| `llm/` | `Agent`, `Caller`, `RawCaller` (adds `CallStructured`), `ToolExecutor`, 6 tool definitions, `NewProviderCaller`/`NewProviderCallerRaw` factory (shared `newProviderCaller` helper), `ProviderConfig`, system prompts, `LlamacppCaller`, `LitertLMCaller`. Tool dispatch via `ToolExecutorPolicy` (`llm/executor.go`, pluggable confirm/execute hooks) shared by shell/CLI/service; `NewConfirmPolicy` helper; `NoopExecutor` runs nothing. Shared `decodeDataURL` in `llm/decode.go`. Agents: `GetAgentDefs`/`GetAgentDef`; `NewAgentFor` intersects agent allowed tools with user toggles; `NewAgentForSession` adds backend + AGENTS.md + skills. `OpenAICaller` has 60s timeout, 10-hop tool limit, explicit `Body.Close` (no defer-in-loop), persists `usage` via `stats.RecordUsage` |
...
- `/models` menu scans `config.GetLlamacppModels()` / `config.GetLitertLMModels()`; `config.IsLlamacppModel()` / `IsLitertLMModel()` used by `SaveModelWithProvider` for provider auto-detection.
 ...
 - `config.GetOpenRouterModels()` is mutex-protected (10 min TTL); `config.LoadCommands` merges home then cwd (cwd wins); `cmd/commands` lists merged commands.
 - `cmd/bot.go`: `splitTelegramMessage` is rune-aware (4096 char limit); `parseAllowList` returns nil for empty allowlist; Telegram poll uses bounded concurrency (10); `sendMessageChunk` validates `json.Marshal`/`ReadAll` errors; `KV` bbolt has 1s timeout.
 - `cmd/shell.go`: `ElaborateMessage` uses `sync.Mutex` for `messages`, explicit `Body.Close` pattern, safe `cancelChan` close, history `Up=+1/Down=-1` with `saveHistory` trailing newline + error handling.
