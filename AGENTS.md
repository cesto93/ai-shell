...
- Auto-updates config: a single file saves the model via `config.SaveModelWithProvider(modelName, provider)`; with two files the first is saved as the model and the second is only downloaded — it is auto-detected as the vision projector by the models-dir scan; each file is downloaded and failures are collected
