...
`ai-shell` is an interactive terminal application that brings AI-powered assistance directly to the command line. It features a rich Text User Interface (TUI) and supports multiple LLM providers, including Ollama (local) and Google Gemini.
...
- **AI Integrations:** Unified OpenAI-compatible API (Ollama, Gemini)

### Architecture
- **`cmd/`**: Contains the CLI entry points and the TUI implementation (`shell.go`).
- **`llm/`**: Provider-specific implementations (`gemini.go`, `ollama.go`) and the `ToolExecutor` interface for AI-driven system interactions.
